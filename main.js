...
Cesium.GoogleMaps.defaultApiKey =
  "AIzaSyA1au3L6n6ZZvFqojyNMfB27DiGHLAX7h8"; // Turn on/off
...
async function main() {
  ...
  slider.addEventListener("input", function (event) {
    ...
    if (currentValue > previousValue) {
      zoomFactor = Math.pow(
        maxZoomFactor,
        Math.abs(currentValue - previousValue) * zoomMultiplier
      );
      viewer.camera.zoomIn(zoomFactor);
    } else if (currentValue < previousValue) {
      zoomFactor = Math.pow(
        minZoomFactor,
        Math.abs(previousValue - currentValue) * zoomMultiplier
      );
